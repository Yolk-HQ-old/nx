...
export function initLocal(workspace: Workspace) {
  const supportedNxCommands = require('@yolkai/nx-workspace/src/command-line/supported-nx-commands')
    .supportedNxCommands;
  ...
  if (supportedNxCommands.includes(process.argv[2])) {
    ...
    require('@yolkai/nx-workspace/src/command-line/nx-commands').commandsObject
      .argv;
  } else {
    if (runOpts === false || process.env.NX_SKIP_TASKS_RUNNER) {
      loadCli(workspace);
    } else {
      require('@yolkai/nx-workspace/src/command-line/run-one').runOne(runOpts);
    }
  }
}
