...
The **@yolkai/nx-angular** package has an `ngrx` schematic to generate files that implement best practices when using NgRx for state management. This schematic generates source files that include enhancements to NgRx for data persistence strategies, and simplified testing.
...
```bash
ng g @yolkai/nx-angular:ngrx <featurename> --module=<path-to-module> --defaults [options]
```
...
```bash
ng g @yolkai/nx-angular:ngrx app --module=apps/<appname>/src/app/app.module.ts --root --minimal
```
...
```bash
ng g @yolkai/nx-angular:lib products
```
...
```bash
ng g @yolkai/nx-angular:ngrx products --module=libs/products/src/lib/products.module.ts --directory +state/products --syntax=creators --defaults
```
